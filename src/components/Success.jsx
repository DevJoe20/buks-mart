...
export default function Success() {
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      try {
        ...
        console.log("Fetching order for session:", sessionId, `(attempt ${retryCount + 1})`);

        const res = await fetch(`/api/order?session_id=${sessionId}`);
        
        // Always parse as JSON first, then check status
        const data = await res.json();

        if (!res.ok) {
          // Handle "Order not found" specifically with retry logic
          if (res.status === 404 && retryCount < 10) { // Increase retries for order not found
        ...
        // ✅ Clear cart only when order is PAID
        if (data.status === "paid") {
          clearCart();
        }


        // If order is still pending, retry after delay (max 10 retries total)
        ...
        // Retry on network errors (max 10 retries total)
        if (retryCount < 10) {
          setTimeout(() => {
            setRetryCount(prev => prev + 1);
          }, 2000);
      ...
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-gray-500">
            {retryCount > 0 ? `Checking payment status... (${retryCount}/10)` : 'Confirming payment...'}
          </p>
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen text-center px-4">
        ...
        <div className="mt-6 space-x-4">
          <Link href="/cart-item" className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
            Back to Cart
          </Link>
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen text-center px-4 py-8">
      ...
      <p className="mt-2 text-gray-600">Thank you for your purchase. We're processing your order.</p>
